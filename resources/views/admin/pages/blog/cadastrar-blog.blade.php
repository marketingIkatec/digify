            ...
            <div class="bg-white shadow-sm sm:rounded-lg h-100">                
                ...
                <div class="p-4 sm:p-8 space-y-6">
                    ...
                    <div class="mt-6 flex items-center gap-2">
                        <a href="{{ route('blogs.index') }}" class="btn btn-secondary">Voltar</a>
                ...
                <div class="upload-body">
                    <div class="preview-area btn-upload image-upload-wrapper position-relative" id="preview-area">
                        @if($imagem = $item->imagem ?? '')
                            <img src="{{ asset('storage/'.$imagem) }}" alt="Imagem de capa do Blog" class="img-fluid">
                        @else
            ...
            <div class="card-right sm:rounded-lg">                
                ...
                <div class="p-3 space-y-6">
                    ...
                    <div class="col-md-12">
                        ...
                        <select id="categorias" name="categorias[]" class="form-control input-selectize" multiple>
                            <option value="">Selecione</option>
                            @foreach(\App\Models\BlogCategoria::all() as $cat)
                                <option value="{{ $cat->id }}" {{ !empty($item) && $item->categorias->contains($cat->id) ? 'selected' : '' }}>{{ $cat->categoria }}</option>
                            @endforeach															
                        </select>	
                    </div>
                    ...
                    <div class="col-md-11">
                        ...
                        @php
                            $autor_id = request('autor_id') ?? ($item->autor_id ?? '');
                        @endphp
                        <select id="autor_id" name="autor_id" class="form-control @error('autor_id') is-invalid @enderror">
                            ...
                            @foreach(\App\Models\BlogAutor::all() as $autor)
                                <option value="{{ $autor->id }}" {{ $autor_id == $autor->id ? 'selected' : '' }}>{{ $autor->autor }}</option>
                            @endforeach 
                        </select>
                    ...
                    <div class="col-md-11">
                        <label for="data_blog" class="form-label">Data de Publicação</label>
                        <input type="date" name="data_blog" class="form-control @error('data_blog') is-invalid @enderror"
                            value="{{ old('data_blog', isset($item) && $item->data_blog ? $item->data_blog->format('Y-m-d') : '') }}">
                            @error('data_blog') <div class="invalid-feedback"> {{ $message }} </div> @enderror
                    </div>
