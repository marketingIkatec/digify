...
@section('content')
    <form action="{{ route('admin.blog.autor.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
                ...
                <div class="p-4 sm:p-8">
                    <div class="row mb-3">
                        <div class="col-md-12">
                            ...
                            <input type="text" name="autor" class="form-control onloadUrl equalsMetaTitle @error('autor') is-invalid @enderror" value="{{ old('autor', $item->autor ?? '') }}">
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea id="ckeditor" name="resumo" rows="10" class="ckeditor form-control" data-upload-url="{{ route('upload.files.ckeditor') }}?_token={{ csrf_token() }}&path=blogs">{{ old('resumo', $item->resumo ?? '') }}</textarea>
                    </div> 
                    
                    <div class="mt-6 flex items-center gap-2">
                        <a href="{{ route('admin.blog.autor.index') }}" class="btn btn-secondary">Voltar</a>
                ...
                <div class="upload-body">
                    <div class="preview-area btn-upload image-upload-wrapper position-relative" id="preview-area">
                        @if($imagem = $item->imagem ?? '')
                            <img src="{{ asset('storage/'.$imagem) }}" alt="Foto do Autor" class="img-fluid">
                        @else
