...
class Filemanager {
	...
	public function __construct($config = '') {
		...
		$config = json_decode($content, true); //$this->error($this->config['options']['fileRoot'], false);
		$Config['options']['fileRoot'] = $config['options']['fileRoot'];
		$config['options']['fileRoot'] .= '/';
		$this->config = $config;
...
private function getFullPath($path = '') {
	...
	//$this->error(sprintf($this->lang('INVALID_VAR '.var_dump($config)),$current_path));	
	if($this->config['options']['fileRoot'] !== false) {
